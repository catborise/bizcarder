...
const { createTestUser, createTestTag, setupSuite, getAuthAgent } = require('../helpers');

const P = 'tags'; // unique prefix for this suite

describe('Tags Routes', () => {
    let user, agent;

    beforeAll(async () => {
        // Create a regular user for tag operations
    ...
    describe('GET /api/tags', () => {
        test('returns all tags for authenticated user', async () => {
            await createTestTag(user.id, { name: `${P}_GetTagsTest` });
        ...
        test('returns tags sorted by name', async () => {
            await createTestTag(user.id, { name: `${P}_ZebraTag` });
            await createTestTag(user.id, { name: `${P}_AppleTag` });
            const res = await agent.get('/api/tags');
            expect(res.status).toBe(200);
    ...
    describe('GET /api/tags/stats', () => {
        test('returns tag usage statistics', async () => {
            await createTestTag(user.id, { name: `${P}_StatsTag` });
        ...
        test('stats entries include id, name, color, and cardCount', async () => {
            await createTestTag(user.id, { name: `${P}_StatsFieldsTag` });
            const res = await agent.get('/api/tags/stats');
            expect(res.status).toBe(200);
    ...
    describe('POST /api/tags', () => {
        test('creates a new tag with name and color', async () => {
            const res = await agent
                .post('/api/tags')
                .send({ name: `${P}_NewTag`, color: '#00ff00' });
            expect(res.status).toBe(201);
            expect(res.body.name).toBe(`${P}_NewTag`);
        ...
        test('creates a tag with default color when color is omitted', async () => {
            const res = await agent
                .post('/api/tags')
                .send({ name: `${P}_DefaultColor` });
            expect(res.status).toBe(201);
            expect(res.body.name).toBe(`${P}_DefaultColor`);
        ...
        test('unauthenticated returns 401', async () => {
            const res = await supertest(app)
                .post('/api/tags')
                .send({ name: `${P}_Ghost`, color: '#000000' });
            expect(res.status).toBe(401);
        });
    });
    ...
    describe('PUT /api/tags/:id', () => {
        test('owner can update their own tag', async () => {
            const tag = await createTestTag(user.id, { name: `${P}_TagToUpdate` });
            const res = await agent
                .put(`/api/tags/${tag.id}`)
                .send({ name: `${P}_Updated`, color: '#0000ff' });
            expect(res.status).toBe(200);
            expect(res.body.name).toBe(`${P}_Updated`);
        ...
        test('rejects update of another users tag with 403', async () => {
            const other = await createTestUser({ username: `${P}_otherupdatetag`, email: `${P}_otherupdatetag@test.com` });
            const tag = await createTestTag(other.id, { name: `${P}_OtherTag` });
            const res = await agent
                .put(`/api/tags/${tag.id}`)
                .send({ name: `${P}_Hacked` });
        ...
        test('unauthenticated returns 401', async () => {
            const tag = await createTestTag(user.id, { name: `${P}_UnauthUpdateTag` });
            const res = await supertest(app)
                .put(`/api/tags/${tag.id}`)
                .send({ name: `${P}_Ghost` });
            expect(res.status).toBe(401);
    ...
    describe('DELETE /api/tags/:id', () => {
        test('owner can delete their own tag', async () => {
            const tag = await createTestTag(user.id, { name: `${P}_TagToDelete` });
        ...
        test('rejects deletion of another users tag with 403', async () => {
            const other = await createTestUser({ username: `${P}_tagowner`, email: `${P}_to@test.com` });
            const tag = await createTestTag(other.id, { name: `${P}_OtherOwnerTag` });
            const res = await agent.delete(`/api/tags/${tag.id}`);
            expect(res.status).toBe(403);
        });

        test('unauthenticated returns 401', async () => {
            const tag = await createTestTag(user.id, { name: `${P}_UnauthDeleteTag` });
